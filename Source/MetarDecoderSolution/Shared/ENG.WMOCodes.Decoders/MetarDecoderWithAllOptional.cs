...
namespace ENG.WMOCodes.Decoders
{
    ...
    public class MetarDecoderWithAllOptional : PublicDecoder<Metar>
    {
        ...
        protected override Metar DecodeCore(string source)
        {
            ...
            var wind = new WindWithVariabilityDecoder { Required = false }.Decode(ref p) ?? new WindWithVariability();
            ret.Wind = wind;

            var visib = new VisibilityForMetarDecoder { Required = false }.Decode(ref p) ?? new VisibilityForMetar();
